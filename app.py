...
@app.get("/")
def read_root():
    return {"status": "✅ API 正在运行，已开启文件名模糊匹配与明细表导出功能！"}

# --- 辅助函数：极致省内存的文件读取方式 ---
async def load_dataframe(file: UploadFile, sheet_name=None):
    name = file.filename.lower()
    
    # 直接使用 file.file 读取流，避免 await file.read() 撑爆 Render 内存
...
async def process_billing(files: List[UploadFile] = File(..., description="一次性上传所有源文件")):
    try:
        ...
        with pd.ExcelWriter(output, engine='xlsxwriter') as writer:
            ...
            # 👇 【新增部分】写入完整的流水分组明细表，专治各种“数字对不上”！
            if not gp_merged.empty:
                gp_merged.to_excel(writer, sheet_name='GoParkin_Details', index=False)
            if not sp_merged.empty:
                sp_merged.to_excel(writer, sheet_name='SP_Details', index=False)
